...
describe("BatchActionsBar", () => {
  ...
  it("renders nothing when no task is selected", () => {
    const { container } = render(<BatchActionsBar count={0} config={null} onRun={() => {}} onClear={() => {}} />);
    expect(container).toBeEmptyDOMElement();
  });

  it("shows the selection count", () => {
    render(<BatchActionsBar count={3} config={null} onRun={() => {}} onClear={() => {}} />);
  ...
  it("runs the approve action against each selected task via onRun", async () => {
    ...
    render(<BatchActionsBar count={2} config={null} onRun={onRun} onClear={() => {}} />);

    await user.click(screen.getByRole("button", { name: "Approve" }));

    expect(onRun).toHaveBeenCalledTimes(1);
    expect(api).toHaveBeenCalledWith("/api/tasks/42/approve", { method: "POST" });
  });
  ...
  it("disables Attach/Detach until a capability is picked", async () => {
    ...
    const user = userEvent.setup();
    render(<BatchActionsBar count={1} config={config} onRun={() => {}} onClear={() => {}} />);
  ...
  it("clears the selection via the clear button", async () => {
    const onClear = vi.fn();
    const user = userEvent.setup();
    render(<BatchActionsBar count={1} config={null} onRun={() => {}} onClear={onClear} />);
