...
// connection at mcpServerPath (a binary built from v2/cmd/mcpserver) with
// -sandbox-root set to RunConfig.SandboxRoot -- nothing in a tool call
// itself can ever change where that lands, mirroring v1's own "only place
// it's named" discipline (dispatch.py's _mcp_config_json).
package claude
...
// (typically just "claude", resolved against $PATH) and points every run's
// --mcp-config at mcpServerPath, a binary built from v2/cmd/mcpserver.
func New(claudePath, mcpServerPath string, opts ...Option) *Framework {
	return newFramework(execRunner{claudePath: claudePath}, mcpServerPath, opts...)
}

func newFramework(run runner, mcpServerPath string, opts ...Option) *Framework {
	f := &Framework{run: run, mcpServerPath: mcpServerPath, maxTurns: defaultMaxTurns}
...
// hand-copied, so this can never drift from what cmd/mcpserver actually
...
// mcpConfigJSON is the --mcp-config file content: mcpServerPath spawned
// with -sandbox-root sandboxRoot.
func mcpConfigJSON(mcpServerPath, sandboxRoot string) ([]byte, error) {
	return json.Marshal(map[string]any{
		"mcpServers": map[string]any{
			mcpServerName: map[string]any{
				"command": mcpServerPath,
				"args":    []string{"-sandbox-root", sandboxRoot},
			},
		},
	})
}
...
// at mcpServerPath and cfg.SandboxRoot, runs claude -p with its native tool
// roster emptied out and only the grain-sandbox MCP tools admitted, and
// parses the resulting --output-format stream-json transcript into a
// Result.
func (f *Framework) Run(ctx context.Context, cfg agent.RunConfig) (*agent.Result, error) {
	...
	if f.mcpServerPath == "" {
		return nil, fmt.Errorf("claude: mcpServerPath is required")
	}
	maxTurns := f.maxTurns
	if cfg.MaxTurns > 0 {
	...
	configJSON, err := mcpConfigJSON(f.mcpServerPath, cfg.SandboxRoot)
	if err != nil {
