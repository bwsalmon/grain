...
# v2/cmd/grain/daemon.go's -kontur-backend) differs by netMode
...
diagnose() {
  ...
  if [ "$net_mode" = "flat" ]; then
    ...
    # translation involved -- v2/scripts/setup.sh's write_systemd_units
  ...
  if [ "$net_mode" = "flat" ]; then
    ...
    if in_netns "$pid" ping -c 2 -W 2 "$ip" >/dev/null 2>&1; then
      ...
      info "kontur-net-cmdline.service oneshot (packer/kontur/provision.sh)."
