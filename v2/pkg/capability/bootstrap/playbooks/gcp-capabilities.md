...
`gemini-key` capabilities need (`v2/pkg/capability/gcpkey`,
`v2/pkg/capability/geminikey`) -- an agent account those capabilities
...
`terraform/gcp-v2/iam.tf`, never at the instance, disk, or network
...
- the zone and `name_prefix` the VM was actually created with (these
  must match `terraform/gcp-v2/example.tfvars`' equivalents exactly, or
...
`terraform/gcp-v2` ships a GCS backend by default
...
cd <path to a checkout of this repo>/terraform/gcp-v2
cp backend-local.hcl.example backend-local.hcl
# edit backend-local.hcl: path = "<GRAIN_DATA_DIR>/terraform-state/gcp-v2.tfstate"
...
state file cover every flow that touches `terraform/gcp-v2`, not one
each.
...
human at `grain setup gcp` (`v2/cmd/grain/setup.go`,
`v2/pkg/gcpsetup`): it does the same identity/IAM work through the GCP
...
variables `v2/scripts/setup.sh` reads), and that the minter account
