...
func fileFixTask(ctx context.Context, store *model.Store, client github.Client,
	...
	// It is also the one position this task can be given, since nobody
	// drags a task grain files for itself: the frontend pins it to the
	// head of the list with no drag handle at all, to match
	// (TaskList.jsx's partitionPinned).
