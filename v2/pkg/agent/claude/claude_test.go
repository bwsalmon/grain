...
// TestRunStripsTheMCPServerPrefixFromToolNames pins the spelling a real
// claude reports an MCP tool under. Every tool loaded from --mcp-config
// is reported as "mcp__grain-sandbox__<tool>" -- allowedTools() above
// builds the very same names for --allowedTools -- so a Result whose
// ToolCalls carried that name unchanged matched nothing downstream:
...
// floor while every test in this file passed, each one having scripted
// the bare name no real run produces.
func TestRunStripsTheMCPServerPrefixFromToolNames(t *testing.T) {
	fake := &fakeRunner{stdout: strings.Join([]string{
		streamJSONLine(t, map[string]any{
			"type": "assistant",
			"message": map[string]any{
				"content": []map[string]any{{
					"type": "tool_use", "id": "call-1", "name": "mcp__grain-sandbox__propose_task",
					"input": map[string]any{"title": "follow-up", "body": "more"},
				}},
			},
		}),
		streamJSONLine(t, map[string]any{
			...
			"message": map[string]any{
				"content": []map[string]any{{
					"type": "tool_result", "tool_use_id": "call-1", "content": "Recorded",
				}},
			},
		}),
		streamJSONLine(t, map[string]any{"type": "result", "result": "proposed one"}),
	}, "\n")}
	f := newFramework(fake, "mcpserver-path")

	result, err := f.Run(context.Background(), agent.RunConfig{Prompt: "propose something", SandboxRoot: t.TempDir()})
	if err != nil {
		t.Fatal(err)
	}
	if len(result.ToolCalls) != 1 {
	...
	if result.ToolCalls[0].Name != "propose_task" {
		t.Errorf("ToolCalls[0].Name = %q, want the tool's own name %q", result.ToolCalls[0].Name, "propose_task")
	}
	if !strings.Contains(result.Transcript, "> propose_task(") {
		t.Errorf("Transcript = %q, want the call rendered under its own name", result.Transcript)
	}
}

// TestRunLeavesAnUnprefixedToolNameAlone is the other half: Run empties
// claude's native tool roster, so a run should have no unprefixed tool
// to report at all, but a name that did
// not come from grain's own MCP server must stay distinguishable from
// one that did rather than being rewritten to look like it.
func TestRunLeavesAnUnprefixedToolNameAlone(t *testing.T) {
	fake := &fakeRunner{stdout: strings.Join([]string{
		streamJSONLine(t, map[string]any{
			"type": "assistant",
			"message": map[string]any{
	...
	result, err := f.Run(context.Background(), agent.RunConfig{Prompt: "run something", SandboxRoot: t.TempDir()})
	if err != nil {
		t.Fatal(err)
	}
	if len(result.ToolCalls) != 1 || result.ToolCalls[0].Name != "Bash" {
		t.Fatalf("ToolCalls = %+v, want one call still named Bash", result.ToolCalls)
	}
}
