...
# --- scratch repos (bwsalmon/agents#159, #186) -----------------------------
#
# No scratch-specific code lives in GitProxy any more (bwsalmon/agents#186):
# a scratch repo (`repo_for_sandbox`'s own naming) is authenticated exactly
# like any other repo, via an `owner/*` (or narrower) `credentials.json`
# pattern an operator adds by hand -- so the only thing worth a regression
# test here is that a scratch-shaped repo really does resolve through the
# ordinary ladder, same as any other.

def test_a_scratch_repo_resolves_through_the_ordinary_owner_wildcard(tmp_path):
    (tmp_path / "repo-allowlist.json").write_text(
        json.dumps(["owner/repo", "owner/grain-scratch-sandbox-0"])
    )
    (tmp_path / "credentials.json").write_text(
        json.dumps({"owner/*": "scratch", "*": "bot"})
    )
    (tmp_path / "bot.token").write_text("bot-token")
    (tmp_path / "scratch.token").write_text("scratch-token")
    (tmp_path / "sandbox-tokens.json").write_text(json.dumps({"sandbox-0": "tok0"}))
...
def test_scratch_repo_still_respects_the_allowlist(tmp_path):
    (tmp_path / "repo-allowlist.json").write_text(
        json.dumps(["owner/grain-scratch-sandbox-0"])
    )
    (tmp_path / "credentials.json").write_text(json.dumps({"owner/*": "scratch"}))
    (tmp_path / "scratch.token").write_text("scratch-token")
    (tmp_path / "sandbox-tokens.json").write_text(json.dumps({"sandbox-0": "tok0"}))
