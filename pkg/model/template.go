...
// "what to do" fields Task and ScheduledTask already carry, saved once
// under a name so more than one place can point at it instead of
// repeating the same title, body, and grants by hand (bwsalmon/
// agents#516). A schedule is the first caller (ScheduledTask.TemplateID);
// nothing about this type itself knows what a schedule is, the same way
// Task itself knows nothing about ScheduledTask -- more callers are
// expected to arrive later without this type changing shape for them.
//
