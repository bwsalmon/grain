...
def _prompt(issue: Issue, branch: str, workspace: str, comments: list[Comment] = (),
    ...
    return (
        ...
        "it for a human reviewer, not just for git log. Before pushing, "
        "rebase onto the latest origin/HEAD (the target repo's current "
        "default branch) so any conflicts are yours to resolve now, while "
        "you still have full context, rather than a human reviewer's to "
        "untangle later:\n"
        "    git fetch origin && git rebase origin/HEAD\n"
        "Resolve any conflicts the rebase reports before continuing. Then "
        "push with exactly this command:\n"
...
def _pr_prompt(pr: PullRequestDetail, comments: list[ReviewComment], workspace: str,
    ...
    return (
        ...
        "When you are done, commit your changes. Before pushing, rebase "
        "onto the latest origin/HEAD (the target repo's current default "
        "branch) so any conflicts are yours to resolve now, while you "
        "still have full context, rather than a human reviewer's to "
        "untangle later:\n"
        "    git fetch origin && git rebase origin/HEAD\n"
        "Resolve any conflicts the rebase reports before continuing. Then "
        "push with exactly this command:\n"
