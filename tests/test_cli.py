...
def test_dry_run_controller_configure_with_a_scratch_repo_owner(capsys):
    out = run(
        ["--dry-run", "controller", "configure", "--repo", "acme/widgets",
         "--scratch-repo-owner", "acme"],
        capsys,
    )
    assert "dd of=/data/config/scratch-repo.json" in out


def test_controller_configure_without_scratch_repo_owner_writes_no_scratch_repo_config(capsys):
    out = run(
        ["--dry-run", "controller", "configure", "--repo", "acme/widgets"],
        capsys,
    )
    assert "scratch-repo.json" not in out
