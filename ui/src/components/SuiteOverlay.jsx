...
export default function SuiteOverlay({
  suite,
  templates = [],
  repoOptions = [],
  config,
  ...
  const [templateIds, setTemplateIds] = useState(
    (suite?.items || []).map((it) => it.templateId),
  );
  ...
  const submit = async (evt) => {
    ...
    const payload = {
      ...
      maxPasses:
        mode === "until_clean" ? Number(data.get("maxPasses")) : undefined,
  ...
  const remove = async () => {
    if (
      !confirm(
        `Delete the suite "${suite.name}"? Runs already started from it are unaffected.`,
      )
    )
      return;
    try {
    ...
    <Overlay onClose={onClose} pane backLabel="Suites">
      ...
      <form className="pane-form" onSubmit={submit}>
          ...
          >
            {templates.map((t) => (
              <MenuItem key={t.id} value={t.id}>
                ...
                <ListItemText
                  primary={t.name}
                  secondary={t.repo ? `${t.title} -- ${t.repo}` : t.title}
                />
              </MenuItem>
            ))}
          </Select>
        ...
        <FormControl fullWidth margin="normal" size="small">
          ...
          <Select
            labelId="suite-mode-label"
            label="Run mode"
            value={mode}
            onChange={(e) => setMode(e.target.value)}
          >
            <MenuItem value="until_clean">
              Run until no repo changes or new tasks
            </MenuItem>
        ...
        {mode === "count" ? (
          ...
          <TextField
            ...
            label="Max passes"
            type="number"
            defaultValue={suite?.maxPasses || 5}
            helperText="stops and reports a failure if no pass ever comes back clean"
            inputProps={{ min: 1 }}
            required
            InputLabelProps={{ required: false }}
            margin="normal"
        ...
        <FormControlLabel
          control={
            <Checkbox
              name="autoMerge"
              defaultChecked={suite ? suite.autoMerge : true}
            />
          }
        ...
        <FormControlLabel
          control={
            <Checkbox
              ...
              defaultChecked={suite?.requireApproval}
            />
          }
        ...
        <Box sx={{ mt: 1 }}>
          <Typography variant="caption" color="text.secondary">
            By default a suite's tasks auto-queue and auto-merge -- check
            "require approval" to review them first.
          </Typography>
        </Box>
