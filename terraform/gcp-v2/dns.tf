...
locals {
  ...
  sslip_dns_name = "${var.name_prefix}-${replace(google_compute_global_address.lb.address, ".", "-")}.sslip.io"

  dns_name = var.dns_managed_zone != "" ? trimsuffix("${var.name_prefix}.${data.google_dns_managed_zone.this[0].dns_name}", ".") : local.sslip_dns_name
}
