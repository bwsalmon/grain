...
class GitHubClient:
    ...
    def create_issue(self, owner: str, repo: str, *, title: str, body: str = "",
        ...
        """Files a new issue on the task repo -- the one write this module
        makes that isn't a reply to something the agent set already did.
        Two independent callers: bwsalmon/agents#83's `core.py`'s
        `_suggest_fix`, filing the follow-up task that fixes an open PR's
        conflicts or failing checks, and bwsalmon/agents#24's
        `_file_feedback_task`, filing a candidate task from PR feedback.
        `labels` lets it land already carrying `needs_approval_label`/
        `triage_label` (`AutomationConfig`) in the same request, rather
        than a second `add_label` call that could fail partway and leave
        the issue unlabelled.
        """
