...
`scripts/setup.sh`'s own unit starts it -- host networking, an
...
class Daemon:
    ...
    The argument list mirrors `scripts/setup.sh`'s own
...
def test_the_image_names_the_sandbox_container_it_expects():
    ...
    what scripts/setup.sh pulls when GRAIN_KONTUR_OCI_IMAGE names
...
def test_a_privileged_port_binds_with_the_capability_and_not_without(tmp_path):
    ...
    from --cap-add alone. Dockerfile gives the grain binary the
